...
describe('Backend: CloudifyCliService Tests', function () {
    ...
    it('test executeCommand', function () {
        ...
        var testOptions = ['pwd','ll'];
        var name = "not a function";

        logger.info('0.1 test exception when file not exist');
        expect( function(){  cloudifyCliService.executeCommand(testOptions, name); } ).toThrow(new Error('executable does not exist ::cloudify-folder/bin/cloudify.sh'));
